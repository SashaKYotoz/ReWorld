...
public class BlueMycenaeMushroom1Feature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public BlueMycenaeMushroom1Feature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(ReworldModBlocks.BLUE_MOSSY_STONE.get(), ReworldModBlocks.BLUE_POMEGRANATE.get());
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("reworld", "blue_mycenae_mushroom"));
		if (template == null)
			return false;
