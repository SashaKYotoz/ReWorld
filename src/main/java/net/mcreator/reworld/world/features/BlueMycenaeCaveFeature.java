...
public class BlueMycenaeCaveFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public BlueMycenaeCaveFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(ReworldModBlocks.BLUE_POMEGRANATE.get(), Blocks.STONE);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("reworld", "blue_mycenae_cave"));
		if (template == null)
			return false;
