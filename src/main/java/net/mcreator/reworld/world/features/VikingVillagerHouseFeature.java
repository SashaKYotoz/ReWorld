...
public class VikingVillagerHouseFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public VikingVillagerHouseFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(Blocks.SNOW_BLOCK, Blocks.PODZOL, Blocks.GRASS_BLOCK);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("reworld", "viking_villager_house"));
		if (template == null)
			return false;
