...
public class WillowTreeFeature extends Feature<NoneFeatureConfiguration> {
	...
	public WillowTreeFeature() {
		...
		base_blocks = List.of(Blocks.WATER, Blocks.CLAY, Blocks.GRASS_BLOCK);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("reworld", "willow_tree"));
		if (template == null)
			return false;
