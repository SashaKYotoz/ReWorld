...
public class TamorindicLunaButterflyEntity extends PathfinderMob {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = TamorindicLunaButterflyEntity.this.getRandom();
				double dir_x = TamorindicLunaButterflyEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = TamorindicLunaButterflyEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = TamorindicLunaButterflyEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public SoundEvent getHurtSound(DamageSource ds) {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.note_block.chime"));
	}
	...
	public static void init() {
		SpawnPlacements.register(ReworldModEntities.TAMORINDIC_LUNA_BUTTERFLY.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
